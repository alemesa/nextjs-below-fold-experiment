...
(function(window) {
  ...
  var _globals = window.GreenSockGlobals || window,
    ...
    _computedStyleScope =
      typeof window !== 'undefined' ? window : _doc.defaultView || { getComputedStyle: function() {} },
      ...
      if (lines) {
        ...
        for (i = 0; i < lines.length; i++) {
          ...
          lineNode.style.cssText =
            'display:block;text-align:' + textAlign + ';position:' + (absolute ? 'absolute;' : 'relative;');
          if (linesClass) {
        ...
        if (specialChars && (testResult = specialChars(text.substr(i), vars.specialChars))) {
          ...
          j =
            ...
            (emojiPair2 >= _emojiModStart && emojiPair2 <= _emojiModEnd)
              ...
              : 2;
          splitText +=
            chars && character !== ' ' ? charStart() + text.substr(i, j) + '</' + tag + '>' : text.substr(i, j);
          i += j - 1;
        } else {
          splitText += chars && character !== ' ' ? charStart() + character + '</' + tag + '>' : character;
        }
